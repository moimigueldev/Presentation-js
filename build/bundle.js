...
function c(){r=q.element,p(r),s(),n()}// MASTER LOOP TO SETUP PRESENTATION CAROUSEL
...
let r;const s=()=>{for(let a,b=0;b<r.children.length;b++)a=r.children[b],a.dataset.slide?(o(a),m(a)):r.removeChild(a)},t=()=>{const a=document.body.children;for(let b=0;b<a.length;b++)return a[b].hasAttribute("presentation-js")?(q.element=a[b],!0):(console.error("Please provide a container with the attribute: presentation-js"),!1)},u=a=>{if(a)for(const b in a)f[b]&&(typeof a[b]==typeof f[b]?f[b]=a[b]:console.error(`invalid value for ${b}`))};return a.start=function(a){return!!t()&&void(u(a),c())},Object.defineProperty(a,"__esModule",{value:!0}),a}({});
